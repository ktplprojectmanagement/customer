...
$where = 'where Vendor_id = ?';
		...
		$data['user_details'] = $this->common_model->fetch_data('customer_details',$where,$this->session->userdata('username'));

//print_r($data['pass_details']);die();
if($id=='')
{
if(isset($data['pass_details']['0']['date']) && date('Y-m-d')>$data['pass_details']['0']['date'])
	...
	$where = 'where Vendor_id = ?';
					//print_r($_POST['customer_id']);die();
			$data['user_details1'] = $this->common_model->fetch_data('customer_details_update',$where,$this->session->userdata('username'));
...
//print_r($data['user_details']);die();
		if (isset($data['user_details']) && count($data['user_details'])>0) {
			$where = 'where Vendor_id = ?';
			$data['user_details1'] = $this->common_model->fetch_data('customer_details',$where,$_POST['vendor_id']);
...
'chk_set' => '2',
	...
	function check()
	{
		...
		if (count($data['edit_user_details'])>0) {
			$where = 'where vendor_id = ?';
				//print_r($_POST['customer_id']);die();
			$data['user_details'] = $this->common_model->fetch_data('customer_details',$where,$_POST['username']);
	...
	function check1()
	{
		...
		if (count($data['edit_user_details'])>0) {
			...
			$where = 'where EMail_id = ?';
			$data['user_details_cust'] = $this->common_model->fetch_data('customer_details',$where,$_POST['username']);
            $where = 'where Sub_Purchaser_Email_ID = ?';
			$data['Sub_user_details1'] = $this->common_model->fetch_data('vendor_details',$where,$_POST['username']);
                $where = 'where Purchaser_Email_ID = ?';
    			$data['Sub_user_details2'] = $this->common_model->fetch_data('vendor_details',$where,$_POST['username']);
    			$where = 'where Sub_Purchaser_Email_ID = ?';
			$data['Sub_user_details11'] = $this->common_model->fetch_data('customer_details',$where,$_POST['username']);
                $where = 'where Purchaser_Email_ID = ?';
    			$data['Sub_user_details21'] = $this->common_model->fetch_data('customer_details',$where,$_POST['username']);
//print_r($data['Sub_user_details1']);die();
 $this->session->set_userdata('vendor_email',$_POST['username']);       
if(count($data['Sub_user_details2'])>0 && count($data['Sub_user_details11'])>0)
{
$this->session->set_userdata('purchaser_detail','2');
$this->session->set_userdata('purchaser_detail1','2');
if(isset($data['user_details']['0']['Vendor_id']))
{
  $this->session->set_userdata('vendor_id',$data['user_details']['0']['Vendor_id']);  
}
}    
else if(count($data['Sub_user_details1'])>0 && count($data['Sub_user_details21'])>0)
{
$this->session->set_userdata('purchaser_detail','1');
$this->session->set_userdata('purchaser_detail1','1');
if(isset($data['user_details']['0']['Vendor_id']))
{
  $this->session->set_userdata('vendor_id',$data['user_details']['0']['Vendor_id']);  
}
...
{
$this->session->set_userdata('purchaser_detail','2');
$this->session->set_userdata('purchaser_detail1','4');
if(isset($data['user_details']['0']['Vendor_id']))
{
  $this->session->set_userdata('vendor_id',$data['user_details']['0']['Vendor_id']);  
}
...
else if(count($data['Sub_user_details1'])>0)
{
$this->session->set_userdata('purchaser_detail','1');
$this->session->set_userdata('purchaser_detail1','4');
if(isset($data['user_details']['0']['Vendor_id']))
...
else if(count($data['Sub_user_details11'])>0)
{
$this->session->set_userdata('purchaser_detail','4');
$this->session->set_userdata('purchaser_detail1','1');
//$this->session->set_userdata('vendor_id',$data['user_details_cust']['0']['Vendor_id']);

}
else if(count($data['Sub_user_details21'])>0)
{
$this->session->set_userdata('purchaser_detail','4');
$this->session->set_userdata('purchaser_detail1','2');
//$this->session->set_userdata('vendor_id',$data['user_details_cust']['0']['Vendor_id']);
}
else if($_POST['username'] == "rajeev.chaubal.admin@vvfltd.com" || $_POST['username'] == "rupesh.acharekar.admin@vvfltd.com")
{
$this->session->set_userdata('purchaser_detail','0');
$this->session->set_userdata('purchaser_detail1','0');
}
else
{
$this->session->set_userdata('purchaser_detail','5');
$this->session->set_userdata('purchaser_detail1','5');
}
