...
$query = "SELECT Vendor_id FROM customer_details
HAVING Vendor_id NOT IN (SELECT Vendor_id FROM customer_details_update)";
...
$data['vendor_details'] = $this->db->query($query)->result_array();
		...
		//print_r(count($data['vendor_detailssssssss']));die();
...
'smtp_crypto' => 'ssl'
 ...
 $this->email->set_mailtype("html");
		   ...
		   $this->email->to($data['user_details']['0']['EMail_id']);  // replace it with receiver mail id
