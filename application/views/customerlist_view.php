						...
						<ul class="nav navbar-nav navbar-right">
							<li class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li><a href="http://kritvainvestments.com/customer/index.php/Adminlogin"><i class="lnr lnr-exit"></i> <span>Logout</span></a></li>
                                </ul>
                            </li>
						</ul>
					</div>
										...
										<?php
											if (isset($vendor_details) && count($vendor_details)>0) {
												for ($i=0; $i < count($vendor_details); $i++) {	
													...
													?>
														<tr>
															<td><input type="checkbox" value="<?php echo $vendor_details[$i]['Vendor_id']; ?>" id="<?php echo $vendor_details[$i]['Vendor_id']; ?>"></td>
															<td><?php echo $vendor_details[$i]['Purchaser_Name']; ?></td>
															...
															<td>
																<form method="post" target="_new" action="<?php echo base_url(); ?>index.php/Vendordata">
																	<input type="text" name="vendor_id" style="display: none" value="<?php echo $vendor_details[$i]['Vendor_id']; ?>" style="display:none">
						...
						//alert($(this).val());
						 var base_url = window.location.origin;
				            $.ajax({
					              type : 'post',
					              data : data,
					              url : base_url+'/customer/index.php/Vendorlist/get_vendor',
						 ...
						 var vendor_value = '';
				          ...
				          var base_url = window.location.origin;
				            $.ajax({
				              type : 'post',
				              data : data,
				              url : base_url+'/customer/index.php/Vendorlist/send_mails',
				              success : function(data)
				              {
				                  alert(data);
				              }
