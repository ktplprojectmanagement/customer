...
'Vendor_id' => $vendor_details[$i]['Vendor_id']
            ...
            $mail_recive_data = $this->common_model->fetch_data('customer_details',$query,$data_check);
            
            $query = "SELECT DISTINCT(vendor_id) FROM mail_record_customer where mail_sent = 1 group by vendor_id";
...
if (vendor_value == '') 
    ...
    window.location.href = 'http://partners.vvfltd.com/customer/index.php/Vendorlist';
}, 1000);
				              }
				            });
					});
...
if($(this).val() != 'on')
{
				          ...
				          var base_url = window.location.origin;
				            $.ajax({
				              type : 'post',
				              data : data,
				              url : base_url+'/customer/index.php/Vendorlist/tech_send_mails',
				              success : function(data)
				              {
				                  alert("Mail Sent Successfully");
				              }
